@props([
    ...
    'titleSize' => 'text-h2M lg:text-h2 font-heebo lg:font-jakarta',
...
<div class="{{ $bgColor }} py-10">
    ...
    <div class="flex justify-center items-center">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 {{ $gap }} w-full max-w-screen-2xl justify-center">
            @foreach ($items as $item)
                @php
                    $link = $item['link'] ?? null;
                @endphp
                @if ($link)
                    <a href="{{ $link }}" class="relative z-10 bg-transparent p-4 rounded-2xl overflow-hidden text-white hover:ring-1 hover:ring-inset hover:ring-primary30 hover:shadow-xl transition-all duration-300">
                @else
                    <div class="relative z-10 bg-transparent p-4 rounded-2xl overflow-hidden text-white hover:ring-1 hover:ring-inset hover:ring-primary30 hover:shadow-xl transition-all duration-300">
                @endif
