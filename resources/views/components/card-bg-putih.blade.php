<a href="{{ $link }}" class="relative block w-[404px] h-[483px] font-jakarta bg-transparent rounded-2xl shadow-md overflow-hidden transition-all group">
    ...
    <div class="relative z-10 bg-transparent p-4 rounded-2xl">
        ...
        <h3 class="text-h3 font-bold text-gray70 mt-2 whitespace-normal">{{ $title }}</h3>
        <div class="flex justify-between items-center mt-2">
            <p class="text-p text-gray70 font-light">{{ $date }}</p>
            <x-label-kategori text="{{ $category }}"/>
        </div>
        <p class="text-gray70 mt-2 text-p text-justify font-light whitespace-normal">
            {{ $description }}
        </p>
