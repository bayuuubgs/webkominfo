<div>
    <a href="{{ $link }}" class="relative block {{ $cardWidth }} h-auto font-responsive bg-transparent rounded-2xl overflow-hidden transition-all group hover:shadow-lg">
        ...
        <div class="relative z-10 bg-transparent p-4 rounded-2xl">
            ...
            <h3 class="text-h3M md:text-h3 font-bold {{ $textColor }} mt-2 whitespace-normal line-clamp-2">
                {{ $title }}
            </h3>

            <div class="flex justify-between items-center mt-2">
                <p class="text-pM md:text-p {{ $dateColor }} font-light">{{ $date }}</p>
                <x-label-kategori text="{{ $category }}" />
            </div>

            <p class="{{ $textColor }} mt-2 text-pM md:text-p text-justify font-light line-clamp-3 whitespace-normal">
                {{ $description }}
            </p>

            <span class="text-blue-400 font-semibold text-pM md:text-p">
